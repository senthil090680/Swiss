  ...
  <tr>
    <td align="left" valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>		
        ...
        <td height="30" align="left" valign="top" class="rtxtr"><strong><?php 
		foreach($country_result[1] as $country_result_value) { if($country_result_value[iso3] == $country_id) { $country_name_stored = $country_result_value[printable_name]; echo $country_result_value[printable_name]; } }
		
		/*foreach($country_array as $coun_arr_key => $coun_arr_val) { if($coun_arr_key == $country_id) { $country_name_stored = $coun_arr_val; echo $coun_arr_val; } } */ ?></strong></td>
        <td align="left" valign="top" class="txt">&nbsp;</td>
      </tr>
      <tr>
    ...
    <?php  
			...
			if($num > 0 ) {
				while($row = mysql_fetch_array($selectResult)) {
					...
					$tour_desc				=		stripslashes($row['tour_desc']);
					$product_services		=		stripslashes($row['product_services']);
					$touring				=		stripslashes($row['touring']);
					$tour_name				=		stripslashes($row['tour_name']);
					$tour_code				=		stripslashes($row['tour_code']);
					$tour_cost				=		stripslashes($row['tour_cost']);
?>
      ...
      <tr>
        ...
        <td align="left" valign="top" class="txt"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="17%" height="144" align="left" valign="top"><a href="javascript:void(0);" onclick="beforeCreateChatBox('<?php echo 'controlend/'.$thumb_path; ?>','<?php echo $tour_id; ?>')"><img src="<?php echo 'controlend/'.$thumb_path; ?>" width="105" height="104" /></a></td>
