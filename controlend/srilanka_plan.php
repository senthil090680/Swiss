...
if($_REQUEST) {
	extract($_REQUEST);
        ...
        $sri_name       =       mysql_real_escape_string(nl2br($sri_name));
        $sri_code       =       mysql_real_escape_string(nl2br($sri_code));
        $sri_ps         =       mysql_real_escape_string(nl2br($sri_ps));
        $sri_touring    =       mysql_real_escape_string(nl2br($sri_touring));
        $sri_desc       =       mysql_real_escape_string(nl2br($sri_desc));
        $sri_cost       =       mysql_real_escape_string(nl2br($sri_cost));
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] != '' ) {
		...
		if($_POST['action'] == 'edit') {
			...
			if($_FILES['sri_path1']['name'] != '') {
				...
				if(file_exists($row1['sri_path1'])) {
					unlink($row1['sri_path1']);
				} else {	
					echo "not deleted"; exit(0);
				}
			...
			if($_FILES['sri_path2']['name'] != '') {
				...
				if(file_exists($row2['sri_path2'])) {
					unlink($row2['sri_path2']);
				} else {	
					echo "not deleted"; exit(0);
				}
			...
			$queryUpdate	=	"UPDATE ".TABLE_SRI." SET sri_name='$sri_name',continent_name='$continent_name',country_name='$country_name',sri_code='$sri_code',sri_ps='$sri_ps',sri_touring='$sri_touring',sri_desc='$sri_desc',sri_cost='$sri_cost',kindtour='$kindtour',sri_front='$sri_front',sri_status='$sri_status',sri_updatedDate=NOW()".$thumbpath.$thumbpath1.$thumbpath2." WHERE sri_id='$sri_id'";			
			...
			if($num > 0) { 	
				$queryUpdate	=	"UPDATE ".TABLE_SRI." set continent_name='$continent_name',country_name='$country_name',sri_ps='$sri_ps',sri_touring='$sri_touring',sri_code='$sri_code',sri_desc='$sri_desc',sri_cost='$sri_cost',kindtour='$kindtour',sri_status='$sri_status',sri_updatedDate=NOW() where sri_name='$sri_name'";			
				...
				$queryInsert	=	"insert into ".TABLE_SRI." (sri_name,continent_name,country_name,sri_code,sri_ps,sri_touring,sri_desc,sri_cost,kindtour,sri_path,sri_path1,sri_path2,sri_status,sri_insertedDate) values ('$sri_name','$continent_name','$country_name','$sri_code','$sri_ps','$sri_touring','$sri_desc','$sri_cost','$kindtour','$thumb_target','$thumb_target1','$thumb_target2','$sri_status',NOW())";
				...
				$selectCount2	=	"SELECT sri_path,sri_path1,sri_path2 from ".TABLE_SRI." where sri_id='$sri_id'";
				$selectResult2	=	mysql_query($selectCount2) or die (mysql_error());
				$num2			=	mysql_num_rows($selectResult2);	
				$row2			=	mysql_fetch_array($selectResult2);
				
				if(file_exists($row2['sri_path'])) {
					unlink($row2['sri_path']);
				} else {	
					//echo $row2['thumb_path']."<br>";
					echo "not deleted 1"; exit(0);
				}
				
				if(file_exists($row2['sri_path1'])) {
					unlink($row2['sri_path1']);
				} else {	
...
<script type="text/javascript">
	...
	function validatePlan() {
        ...
        var kindtour            =	document.getElementById('kindtour');
		...
		if(sri_name.value == '') {
			...
			document.getElementById('kindtourSpan').innerHTML						=	'';
		...
		/*else if(sri_desc.value == '') {
			...
			return false;
                ...
                else if(kindtour.value == '') {
			document.getElementById('kindtourSpan').innerHTML						=	'Please choose the kind of tour';
			document.getElementById('sri_nameSpan').innerHTML						=	"";
			/*document.getElementById('continent_nameSpan').innerHTML				=	"";
			document.getElementById('country_nameSpan').innerHTML					=	'';*/
			document.getElementById('sri_codeSpan').innerHTML						=	'';
		...
		if(action_find.value == 'new') {
			...
			if(sri_path1.value == '') {
			document.getElementById('sri_path1Span').innerHTML						=	'Please pick the thumb image';
			document.getElementById('sri_nameSpan').innerHTML						=	"";
			/*document.getElementById('continent_nameSpan').innerHTML				=	"";
			document.getElementById('country_nameSpan').innerHTML					=	'';*/
			document.getElementById('sri_codeSpan').innerHTML						=	'';
			...
			return false;
				...
				document.getElementById('sri_path2Span').innerHTML					=	'Please pick the thumb image';
				document.getElementById('sri_nameSpan').innerHTML					=	"";
				/*document.getElementById('continent_nameSpan').innerHTML			=	"";
				document.getElementById('country_nameSpan').innerHTML				=	'';*/
				document.getElementById('sri_codeSpan').innerHTML					=	'';
	...
	function editPlan(sri_id){
		...
		d.sri_cost.value								=	document.getElementById('sri_cost-'+sri_id).innerHTML;//Tour Cost
                d.kindtour.value						=	document.getElementById('kindtour-'+sri_id).innerHTML;//Kind of Tour 
			...
			<form name="plan_form" action="srilanka_plan.php" method="post" onsubmit="return validatePlan();" enctype='multipart/form-data'>
				<fieldset>
					...
					<p>
						...
						<textarea  style="width: 590px;" name="sri_desc" id="sri_desc" class="text-long"  ></textarea>
					</p>
					<p>
					...
					<p>
						...
						<textarea style="width: 550px;" name="sri_ps" id="sri_ps" class="text-long"  ></textarea>
					</p>
					<p>
					...
					<p>
						...
						<textarea style="width: 570px;" name="sri_touring" id="sri_touring" class="text-long"  ></textarea>
					</p>
					<p>
					...
					<p>
						<span id='sri_costSpan' class='backvalid'></span>
                                        ...
                                        <p>
						<label style="padding-right:20px;">Kind Of Tour</label>:&nbsp;&nbsp;
                                        
                                        <select name="kindtour" id="kindtour" class="input" style="width:200px">
					...
					<p>
						<span id='kindtourSpan' class='backvalid'></span>
					</p>
...
<tr>
	<td>
		<table class="tablecss" border='1'>
			...
			$result = SingleTon::selectQuery("sri_id,sri_name,continent_name,country_name,sri_ps,sri_touring,sri_code,sri_desc,sri_cost,kindtour,sri_path,sri_path1,sri_path2,sri_status","ORDER BY sri_id DESC",TABLE_SRI,MY_ASSOC,NOR_YES);
			...
			if($num_rows > 0) {
				...
				foreach($result[1] as $result){
					...
					$sri_desc				=	str_replace(array("\r", "\n", "<br>","<br />","<br/>","</br>","</ br>"),'',$result["sri_desc"]);
					$sri_ps					=	str_replace(array("\r", "\n", "<br>","<br />","<br/>","</br>","</ br>"),'',$result["sri_ps"]);
					$sri_touring			=	str_replace(array("\r", "\n", "<br>","<br />","<br/>","</br>","</ br>"),'',$result["sri_touring"]);
					$sri_cost				=	$result["sri_cost"];
                                        
                                        $kindtour                               =	$result["kindtour"];
					$sri_status				=	$result["sri_status"];
					$sri_path				=	$result["sri_path"];
					$sri_path1				=	$result["sri_path1"];
					$sri_path2				=	$result["sri_path2"];
                                        
                                        if(is_null($kindtour) || $kindtour == '') {                                                                                   $kindtour = '';    
                                        }
					$row_count++;
			...
			<tr <?php echo $row_class;?>>      
				...
				<div style="display:none;" id="sri_cost-<?php echo $sri_id; ?>"><?php echo $sri_cost; ?></div>
                                <div style="display:none;" id="kindtour-<?php echo $sri_id; ?>"><?php echo $kindtour; ?></div>
