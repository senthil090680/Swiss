	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] != '' ) {
		...
		if($_POST['action'] == 'edit') {
			...
			if($_FILES['thumb_path1']['name'] != '') {
				...
				if(file_exists($row['thumb_path1'])) {
					unlink($row['thumb_path1']);
				} else {	
					echo "not deleted";
				}
			...
			if($_FILES['thumb_path2']['name'] != '') {
				...
				if(file_exists($row['thumb_path2'])) {
					unlink($row['thumb_path2']);
				} else {	
					echo "not deleted";
				}
			...
			if($num > 0) { 	
				$queryUpdate	=	"UPDATE ".TABLE_PLAN." set continent_name='$continent_name',country_name='$country_name',product_services='$product_services',touring='$touring',tour_code='$tour_code',tour_desc='$tour_desc',tour_cost='$tour_cost',plan_status='$plan_status',updatedDate=NOW() where tour_name='$tour_name'";			
				...
				$queryInsert	=	"insert into ".TABLE_PLAN." (tour_name,continent_name,country_name,tour_code,product_services,touring,tour_desc,tour_cost,thumb_path,thumb_path1,thumb_path2,plan_status,insertedDate) values ('$tour_name','$continent_name','$country_name','$tour_code','$product_services','$touring','$tour_desc','$tour_cost','$thumb_target','$thumb_target1','$thumb_target2','$plan_status',NOW())";
...
<tr>
	<td>
		<table class="tablecss" border='1'>
			...
			$result = SingleTon::selectQuery("tour_id,tour_name,continent_name,country_name,product_services,touring,tour_code,tour_desc,tour_cost,thumb_path,thumb_path1,thumb_path2,plan_status","ORDER BY tour_id DESC",TABLE_PLAN,MY_ASSOC,NOR_YES);
